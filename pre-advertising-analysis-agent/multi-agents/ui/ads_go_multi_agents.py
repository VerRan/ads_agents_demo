...
# process_sequential_workflow 
# 直接使用agent的输出作为下一个agent的输出，将agent编排为一个按顺序执行的流程
    # Sequential workflow processing
def process_sequential_workflow(topic):
    # Step 1: product_results
    product_results = product_analyst(f" {topic}")
    # Step 2: audience_analysis
    audience_analysis = audience_analyst(f" {product_results}")

    # Step 3: market_analysis
    market_analysis = market_analyst(f" {audience_analysis}")
    # Step 3: competitor_analysis
    competitor_analysis = competitor_analyst(f" {market_analysis}")

    writer = Agent(system_prompt="You create polished reports based on analysis.")

    # Step 3: Report writing
    final_report = writer(f"Create a report based on this analysis: {product_results,audience_analysis,market_analysis,competitor_analysis}")
...
def main():
    # process_agent_as_tool("https://www.kreadoai.com")
    process_sequential_workflow("https://www.kreadoai.com")
